...
function peopleReducer(state = initialState, action) {
    ...
    switch (action.type) {
        case "ADD_PEOPLE_TO_LIST": {
            const isExist = state.addedPeople.find(item=> item.id === action.payload.id)
            if(isExist){
                return state;
            }
        ...
        case "REMOVE_PEOPLE_FROM_LIST": {
            const newState = { ...state, addedPeople: state.addedPeople.filter(people => people.id !== action.payload.id), removedPeople: [...state.removedPeople, action.payload] };
            return newState
        }
        default:
            return state;
